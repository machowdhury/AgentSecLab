"""Dashboard Studio contracts for LAB-MCP-CATALOG WS-MCP-CATALOG.
...
CATALOG_DIR = LAB_DIR / "searches"
DEFINITION_PATH = LAB_DIR / "dashboard.definition.json"
VIEW_XML = (
    ROOT / "splunk_app" / "agentsec" / "default" / "data" / "ui" / "views" / "ws_lab_mcp_catalog.xml"
)
NAV_XML = ROOT / "splunk_app" / "agentsec" / "default" / "data" / "ui" / "nav" / "default.xml"
BUILDER = ROOT / "scripts" / "build_lab_mcp_catalog_dashboard.py"
...
def test_definition_files_exist_and_match():
    ...
    assert "ws_lab_mcp_catalog" in NAV_XML.read_text(encoding="utf-8")
...
def test_datasources_are_validated_spl_with_token_bind_only():
    ...
    catalog_lab = json.loads((CATALOG_DIR / "catalog.json").read_text(encoding="utf-8"))
    catalog_query = catalog_lab["queries"][0]
    assert catalog_query["id"] == "Q-MCP-CATALOG-AUTHORITY"
    expected = {
        ...
        "ds_q_catalog": (CATALOG_DIR / catalog_query["spl_file"], "run_id"),
        ...
        "ds_q_executed_baseline": (
            SEARCH_DIR / queries["Q-MCP-EXECUTED"]["spl_file"],
            "baseline_run_id",
        ),
        ...
        "ds_what_baseline": (CATALOG_DIR / catalog_query["spl_file"], "baseline_run_id"),
        "ds_what_attack": (CATALOG_DIR / catalog_query["spl_file"], "attack_run_id"),
        "ds_what_retest": (CATALOG_DIR / catalog_query["spl_file"], "retest_run_id"),
    }
...
def test_display_searches_do_not_drift_from_validated_filter():
    ...
    assert "agentsec.mcp.metadata.trust" in seq
    assert "agentsec.mcp.metadata.provenance" in seq
    assert "agentsec.content.hash" in seq
    ...
    assert "content.preview" not in seq
    assert "_raw" not in seq
    query = definition["dataSources"]["ds_observe_seq"]["options"]["query"]
...
def test_no_duplicate_searches_and_security_semantics():
    ...
    assert "Q-MCP-CATALOG-AUTHORITY" in blob
    ...
    assert "DETECTION CANDIDATE JUSTIFIED FOR LATER DESIGN" in markdown
    assert "DETECTION GAP" in markdown
    assert "No DET-MCP-CATALOG" in markdown or "**No DET-MCP-CATALOG.**" in markdown
    assert "INTENTIONALLY VULNERABLE" in markdown
    assert "not published" in markdown.lower()
    assert "Q-MCP-CATALOG-METADATA" in markdown
    assert "schema" in markdown.lower() and "1.5.0" in markdown
    assert "vulnerable_profile_fail_open:metadata_derived_authority" in markdown
    assert "tool_not_granted" in markdown
    assert "metadata_is_data" in markdown
    ...
    assert not any("catalog_metadata" in name.lower() for name in ds_names)
    assert not any("det_mcp_catalog" in name.lower() for name in ds_names)
...
def test_table_empty_copy_is_nodata_not_caption():
    for viz_id, viz in _definition()["visualizations"].items():
        ...
        assert "attack blocked" not in lowered
        assert "tool safe" not in lowered
    markdown = "\n".join(
        viz["options"]["markdown"]
        for viz in _definition()["visualizations"].values()
        if viz["type"] == "splunk.markdown"
    ...
    assert SPECIMEN_IDS["baseline_run_id"] in learn
    assert SPECIMEN_IDS["attack_run_id"] in learn
    assert SPECIMEN_IDS["retest_run_id"] in learn
    assert "Do not read that as metadata executed" in json.dumps(_definition())


def test_what_happened_is_catalog_authority():
    definition = _definition()
    for ds_id in ("ds_what_baseline", "ds_what_attack", "ds_what_retest", "ds_q_catalog"):
        query = definition["dataSources"][ds_id]["options"]["query"]
        assert "CTRL-MCP-METADATA-001" in query
        assert "metadata_trust" in query or "agentsec.mcp.metadata.trust" in query
        assert "no_indexed_followon_execution_event" in query
    ...
    assert viz["viz_prove_what"]["dataSources"]["primary"] == "ds_q_catalog"


def test_specimen_uuids_and_hash_equality_are_complete():
    ...
    compare = next(
        viz["options"]["markdown"]
        for viz in _definition()["visualizations"].values()
        if viz.get("title") == "BEFORE / AFTER"
    )
    assert MALICIOUS_HASH in compare
    assert "SAME" in compare
    retest = next(
        viz["options"]["markdown"]
        for viz in _definition()["visualizations"].values()
        if viz.get("title") == "STEP 7 RETEST"
    )
    assert MALICIOUS_HASH in retest
    assert "equals ATTACK" in json.dumps(_definition()) or "ATTACK hash == RETEST hash" in retest
...
def test_q_mcp_source_files_unchanged_by_bind():
    ...
    catalog = (CATALOG_DIR / "Q-MCP-CATALOG-AUTHORITY.spl").read_text(encoding="utf-8")
    assert "__RUN_ID__" in catalog
    assert catalog.strip() != _definition()["dataSources"]["ds_q_catalog"]["options"]["query"]
    executed = (SEARCH_DIR / "Q-MCP-EXECUTED.spl").read_text(encoding="utf-8")
    assert "eventstats" in executed
    assert executed.strip() == (SEARCH_DIR / "Q-MCP-EXECUTED.spl").read_text(encoding="utf-8").strip()
...
def test_rejected_spl_is_not_a_datasource():
    ...
    assert "DET-MCP-CATALOG" not in names
