"""Dashboard Studio contracts for LAB-MCP-003 WS-MCP-003.
...
VIEW_XML = (
    ROOT / "splunk_app" / "agentsec" / "default" / "data" / "ui" / "views" / "ws_lab_mcp_003.xml"
)
NAV_XML = ROOT / "splunk_app" / "agentsec" / "default" / "data" / "ui" / "nav" / "default.xml"
BUILDER = ROOT / "scripts" / "build_lab_mcp_003_dashboard.py"
...
FORBIDDEN_CLAIMS = (
    ...
    "no splunk row = deny",
    "unknown = deny",
    "unknown_scope is deny",
    "known-but-ungranted = unknown",
    "tool granted = scope granted",
    "allowed_scope changed",
    "splunk authorized",
    "scope escalation was blocked",
)
...
def test_definition_files_exist_and_match():
    ...
    assert "ws_lab_mcp_003" in NAV_XML.read_text(encoding="utf-8")
...
def test_datasources_are_validated_spl_with_token_bind_only():
    ...
    sim_spl = (SEARCH_DIR / fixture["spl_file"]).read_text(encoding="utf-8").strip()
    assert definition["dataSources"]["ds_det_mcp_001_scope_sim"]["options"]["query"] == sim_spl
    extra = {
...
def test_no_duplicate_searches_and_security_semantics():
    ...
    assert "DET-MCP-003.spl" not in blob
    assert "Q-MCP-003" not in blob
    ...
    assert "DET-MCP-001-SCOPE-POSITIVE-CONTROL" in blob
    ...
    assert "May this agent call this tool" in markdown
    assert "requested scope" in markdown.lower()
    assert "policy:restricted:read" in markdown
    assert "policy:write" in markdown
    assert "unknown_scope" in markdown
...
def test_table_empty_copy_is_nodata_not_caption():
    for viz_id, viz in _definition()["visualizations"].items():
        ...
        assert viz.get("hideWhenNoData") is False
        assert "scope escalation was blocked" not in no_data.lower()
    markdown = "\n".join(
        viz["options"]["markdown"]
        for viz in _definition()["visualizations"].values()
        if viz["type"] == "splunk.markdown"
    ...
    assert learn.index("5b089682-1d5a-49a7-ac43-967265fd6bc6") < learn.index("Authorization path")
