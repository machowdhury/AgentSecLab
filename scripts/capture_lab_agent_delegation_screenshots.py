#!/usr/bin/env python3
"""Capture LAB-AGENT-DELEGATION-001 Dashboard Studio screenshots from Splunk Web.
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=str(ROOT / "docs" / "screenshots" / "lab-agent-delegation"),
    ...
    report: dict = {
        "url": "http://127.0.0.1:8000/en-US/app/agentsec/ws_lab_agent_delegation",
        "tabs_found": [],
        "tabs_missing": [],
        "tokens_found": [],
        "tokens_missing": [],
    ...
    with sync_playwright() as p:
        ...
        atk_dir = ROOT / "docs" / "screenshots" / "attack-service-identity"
        atk_dir.mkdir(parents=True, exist_ok=True)
        for width in WIDTHS:
            page.set_viewport_size({"width": width, "height": 1100})
            resp = page.goto(
                "http://127.0.0.1:5001/labs/LAB-AGENT-DELEGATION-001",
        ...
        page.screenshot(path=str(png), full_page=True)
        report["screenshots"].append(str(png.relative_to(ROOT)))
        report["attack_service"]["attack_run_id"] = page.locator("#attack-run-id").input_value()
        ...
        page.screenshot(path=str(png), full_page=True)
        report["screenshots"].append(str(png.relative_to(ROOT)))
        report["attack_service"]["retest_run_id"] = page.locator("#retest-run-id").input_value()
