#!/usr/bin/env python3
"""Capture LAB-AGENT-GOAL-INTEGRITY-001 Dashboard Studio screenshots from Splunk Web.
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=str(ROOT / "docs" / "screenshots" / "lab-agent-goal-integrity"),
    ...
    with sync_playwright() as p:
        ...
        atk_dir = ROOT / "docs" / "screenshots" / "attack-service-goal"
        atk_dir.mkdir(parents=True, exist_ok=True)
        for width in WIDTHS:
            page.set_viewport_size({"width": width, "height": 1100})
            resp = page.goto(
                "http://127.0.0.1:5001/labs/LAB-AGENT-GOAL-INTEGRITY-001",
