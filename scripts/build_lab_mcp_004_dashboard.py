...
def build() -> dict:
    ...
    q_res_b = bind_run_id(load_spl("Q-MCP-RESOURCE-AUTHZ.spl", resource=True), "baseline_run_id")
    q_res_a = bind_run_id(load_spl("Q-MCP-RESOURCE-AUTHZ.spl", resource=True), "attack_run_id")
    q_res_r = bind_run_id(load_spl("Q-MCP-RESOURCE-AUTHZ.spl", resource=True), "retest_run_id")
    q_res_u = bind_run_id(load_spl("Q-MCP-RESOURCE-AUTHZ.spl", resource=True), "unknown_run_id")
...
**WS-MCP-004** · GUIDED · schema agentsec.security_event **1.2.0** · INV-001 · MCP-004 · CTRL-MCP-001
...
## Validated specimen ids (Phase 5C LIVE)
...
**Control:** CTRL-MCP-001 (src/agentsec/mcp/authorize.py). Same control as LAB-MCP-001 / LAB-MCP-003. No CTRL-MCP-004.
...
Questions live in learning/level_1/LAB-MCP-004/knowledge-check.md. Sample:
...
""",
    ...
    definition = {
        "title": "LAB-MCP-004 Parameter and Resource Authorization",
        "description": (
...
def write_xml(definition: dict) -> None:
    ...
    xml = (
        ...
        "  <label>LAB-MCP-004 Parameter and Resource Authorization</label>\n"
        "  <description>WS-MCP-004. Validated Q-MCP SPL plus Q-MCP-RESOURCE-AUTHZ. DET-MCP-001 packaged disabled. Not DET-MCP-004. Not a notable-event pack. Splunk does not ALLOW or DENY a resource.</description>\n"
        "  <definition><![CDATA[\n"
        f"{payload}\n"
        "  ]]></definition>\n"
        "</dashboard>\n"
