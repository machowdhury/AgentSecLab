#!/usr/bin/env python3
"""Capture Phase 17B fresh-learner surfaces.

Does not print credentials. Does not launch attacks.
"""

from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
WIDTHS = (1440, 1280, 1024)
HEIGHT = 1100
HOME = "http://127.0.0.1:8000/en-US/app/agentsec/ws_agentsec_home"
MASTERY = "http://127.0.0.1:8000/en-US/app/agentsec/ws_agentsec_mastery"
ATTACK = "http://127.0.0.1:5001/"
MENUS = ("Foundations", "Context Security", "Agent Intent", "Capstone")
LABS = {
    "pi": "http://127.0.0.1:8000/en-US/app/agentsec/ws_lab_pi_001",
    "mcp": "http://127.0.0.1:8000/en-US/app/agentsec/ws_lab_mcp_001",
    "rag": "http://127.0.0.1:8000/en-US/app/agentsec/ws_lab_rag_context",
    "memory": "http://127.0.0.1:8000/en-US/app/agentsec/ws_lab_memory_security",
    "goal": "http://127.0.0.1:8000/en-US/app/agentsec/ws_lab_agent_goal_integrity",
    "identity": "http://127.0.0.1:8000/en-US/app/agentsec/ws_lab_agent_delegation",
    "capstone": "http://127.0.0.1:8000/en-US/app/agentsec/ws_lab_agentsec_capstone",
}
ATTACK_TABS = {
    "goal": ("ATTACK",),
    "capstone": ("ATTACK", "PROVE"),
    "memory": ("ATTACK",),
    "pi": ("HUNT",),
    "mcp": ("DEFEND",),
}


def load_env_value(key: str) -> str:
    env_path = ROOT / ".env"
    for raw in env_path.read_text(encoding="utf-8").splitlines():
        line = raw.strip()
        if not line or line.startswith("#") or "=" not in line:
            continue
        name, value = line.split("=", 1)
        if name.strip() == key:
            return value.strip().strip('"').strip("'")
    return ""


def click_tab(page, tab: str) -> bool:
    loc = page.get_by_role("tab", name=tab)
    if loc.count() == 0:
        loc = page.get_by_text(tab, exact=True)
    if loc.count() == 0:
        return False
    loc.first.click()
    return True


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--out",
        default=str(ROOT / "docs" / "screenshots" / "agentsec-academy-17b"),
    )
    parser.add_argument("--label", default="pass17b")
    args = parser.parse_args()
    out = Path(args.out)
    out.mkdir(parents=True, exist_ok=True)

    try:
        from playwright.sync_api import sync_playwright
    except ImportError:
        print("ERROR: playwright is not installed", file=sys.stderr)
        return 2

    password = load_env_value("SPLUNK_PASSWORD")
    if not password:
        print("ERROR: SPLUNK_PASSWORD missing from .env", file=sys.stderr)
        return 2

    report: dict = {
        "label": args.label,
        "viewports": list(WIDTHS),
        "screenshots": [],
        "nav_labels": [],
        "home_tabs": [],
        "labs": [],
        "attack_where": False,
        "defects": [],
    }

    with sync_playwright() as p:
        browser = p.chromium.launch(headless=True)
        page = browser.new_page(viewport={"width": 1440, "height": HEIGHT})
        page.goto("http://127.0.0.1:8000/en-US/account/login", wait_until="domcontentloaded")
        page.locator("input[name='username']").fill("admin")
        page.locator("input[name='password']").fill(password)
        page.locator("input[type='submit'], button[type='submit']").first.click()
        page.wait_for_url("**/app/**", timeout=60000)

        page.goto(HOME, wait_until="domcontentloaded")
        page.wait_for_timeout(4000)
        body = page.inner_text("body")
        if "not published" in body.lower():
            report["defects"].append("BLOCKER Home still says identity not published")
        if "If you already know agents" not in body:
            report["defects"].append("HIGH Home missing Persona C skip")
        if "Start here" not in body and "START LEARNING" not in body:
            report["defects"].append("HIGH Home missing Start here")

        for width in WIDTHS:
            page.set_viewport_size({"width": width, "height": HEIGHT})
            page.wait_for_timeout(400)
            shot = out / f"{args.label}_home_{width}.png"
            page.screenshot(path=str(shot), full_page=True)
            report["screenshots"].append(str(shot.relative_to(ROOT)))

        page.set_viewport_size({"width": 1440, "height": HEIGHT})
        for tab in ("START", "ORIENT", "PATH", "SPLUNK"):
            if click_tab(page, tab):
                page.wait_for_timeout(800)
                report["home_tabs"].append(tab)
                shot = out / f"{args.label}_home_tab_{tab.lower()}.png"
                page.screenshot(path=str(shot), full_page=True)
                report["screenshots"].append(str(shot.relative_to(ROOT)))
                tab_text = page.inner_text("body")
                if tab == "ORIENT" and "source_run_id" not in tab_text:
                    report["defects"].append("HIGH Home ORIENT missing source_run_id")
                if tab == "ORIENT" and "Fingerprint" not in tab_text:
                    report["defects"].append("HIGH Home ORIENT missing Fingerprint")

        for menu in MENUS:
            loc = page.get_by_text(menu, exact=True)
            if loc.count() == 0:
                report["defects"].append(f"HIGH missing nav collection {menu}")
                continue
            loc.first.click()
            page.wait_for_timeout(400)
            report["nav_labels"].append(menu)
            page.keyboard.press("Escape")

        for key, url in LABS.items():
            page.goto(url, wait_until="domcontentloaded")
            page.wait_for_timeout(3500)
            report["labs"].append(key)
            lab_text = page.inner_text("body")
            if key == "goal" and "LAB-AGENT-GOAL" in lab_text[:400] and "Goal" not in lab_text:
                report["defects"].append("MEDIUM Goal LEARN still leads with internal lab id")
            shot = out / f"{args.label}_{key}_1440.png"
            page.screenshot(path=str(shot), full_page=True)
            report["screenshots"].append(str(shot.relative_to(ROOT)))
            for tab in ATTACK_TABS.get(key, ()):
                if click_tab(page, tab):
                    page.wait_for_timeout(1200)
                    tab_text = page.inner_text("body")
                    shot = out / f"{args.label}_{key}_{tab.lower()}_1440.png"
                    page.screenshot(path=str(shot), full_page=True)
                    report["screenshots"].append(str(shot.relative_to(ROOT)))
                    if tab == "ATTACK" and key in {"goal", "capstone"}:
                        if "WHY ARE WE DOING THIS?" not in tab_text:
                            report["defects"].append(f"HIGH {key} ATTACK missing WHY")
                        if "WHAT DO YOU PREDICT?" not in tab_text:
                            report["defects"].append(f"HIGH {key} ATTACK missing PREDICT")
                    if tab == "PROVE" and key == "capstone" and "Mastery Check" not in tab_text:
                        report["defects"].append("HIGH Capstone PROVE missing Mastery Check")
                    if tab == "ATTACK" and key == "memory" and "Launch ATTACK (LIVE)" not in tab_text:
                        report["defects"].append("HIGH Memory ATTACK missing Launch ATTACK (LIVE)")
                else:
                    report["defects"].append(f"HIGH missing {key} tab {tab}")

        page.goto(MASTERY, wait_until="domcontentloaded")
        page.wait_for_timeout(3500)
        mastery = page.inner_text("body")
        if "not a certificate" not in mastery.lower():
            report["defects"].append("HIGH Mastery Check missing not-a-certificate copy")
        for width in WIDTHS:
            page.set_viewport_size({"width": width, "height": HEIGHT})
            page.wait_for_timeout(400)
            shot = out / f"{args.label}_mastery_{width}.png"
            page.screenshot(path=str(shot), full_page=True)
            report["screenshots"].append(str(shot.relative_to(ROOT)))
        page.set_viewport_size({"width": 1440, "height": HEIGHT})
        if click_tab(page, "FOUNDATIONAL"):
            page.wait_for_timeout(900)
            found = page.inner_text("body")
            if "You do not need Splunk Search" not in found:
                report["defects"].append("HIGH Mastery FOUNDATIONAL still requires Search on NONE")
            shot = out / f"{args.label}_mastery_tab_foundational.png"
            page.screenshot(path=str(shot), full_page=True)
            report["screenshots"].append(str(shot.relative_to(ROOT)))

        page.goto(ATTACK, wait_until="domcontentloaded")
        page.wait_for_timeout(1500)
        attack_text = page.inner_text("body")
        report["attack_where"] = "Where you are" in attack_text
        if not report["attack_where"]:
            report["defects"].append("HIGH Attack Service missing Where you are")
        if "EVIDENCE READY" not in attack_text:
            report["defects"].append("HIGH Attack Service missing EVIDENCE READY wait copy")
        if "Empty Search is not DENY" not in attack_text:
            report["defects"].append("HIGH Attack Service missing empty-search teaching")
        for width in WIDTHS:
            page.set_viewport_size({"width": width, "height": HEIGHT})
            page.wait_for_timeout(300)
            shot = out / f"{args.label}_attack_{width}.png"
            page.screenshot(path=str(shot), full_page=True)
            report["screenshots"].append(str(shot.relative_to(ROOT)))

        browser.close()

    (out / f"{args.label}_validation.json").write_text(
        json.dumps(report, indent=2) + "\n", encoding="utf-8"
    )
    print(json.dumps(report, indent=2))
    blockers = [d for d in report["defects"] if d.startswith("BLOCKER") or d.startswith("HIGH")]
    return 1 if blockers else 0


if __name__ == "__main__":
    raise SystemExit(main())
